...
import { playerOne, playerTwo, ball, Point, Vector2,
    applyBallHorizontalBounce, drawPlayerScores, arenaWidth, clientTick,
    roundMax, handlePaddleCollision, assertIsNotNull,
    textColor, intervals } from './lib'
import { DifficultyLevel } from './ai'
...
export async function gameOfflineLobby(
    ...
    function cleanup() {
        ...
        if (intervals.gameOfflineUpdate !== undefined) {
            clearInterval(intervals.gameOfflineUpdate)
            intervals.gameOfflineUpdate = undefined
        }
    }

    function endGame() {
    // playerOne = YOU (right), playerTwo = BOT (left)
    const youWon = playerOne.roundScore >= roundMax;

    const winnerLabel = youWon ? 'YOU WIN!' : 'YOU LOST!';
    const leftLabel = 'BOT';
    ...
    window.dispatchEvent(new CustomEvent('game-over', {
        detail: {
            ...
            winnerLabel,
            resultLabel: youWon ? 'win' : 'loss',
            scorePlayer1: playerTwo.roundScore, // BOT
            scorePlayer2: playerOne.roundScore, // YOU
            player1Label: leftLabel,
            player2Label: rightLabel,
        }
    }));
}
