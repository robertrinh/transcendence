...
function removeStaleQueueEntries(entry) {
...
export function dbCleanUpJob() {
    setInterval(() => {
        ...
        const cleanup = db.transaction(() => {
            const staleQueueEntries = db.prepare('SELECT player_id FROM game_queue WHERE lobby_id IS NULL AND joined_at < ?').all(staleTimeRandom);
            for (const entry of staleQueueEntries) {
                removeStaleQueueEntries(entry);
            }
            const staleQueueEntriesLobby = db.prepare('SELECT player_id FROM game_queue WHERE lobby_id IS NOT NULL AND joined_at < ?').all(staleTimeLobby);
            for (const entry of staleQueueEntriesLobby) {
                removeStaleQueueEntries(entry);
            }
            const staleGames = db.prepare(`SELECT id, player1_id, player2_id FROM games WHERE status NOT IN ('finished', 'cancelled') AND finished_at IS NULL AND created_at < (datetime('now', '-2 minutes'))`).all();
