...
async function registerGuest(reply, username) {
    const result = db.prepare('INSERT INTO users (username, is_guest) VALUES (?, ?)')
        .run(username, 1);
...
async function registerUser(reply, username, password, email) {
    if (!email) {
...
export default async function authRoutes(fastify, options) {
    fastify.post('/auth/login', {
        ...
        const is2faEnabled = db.prepare('SELECT two_factor_enabled FROM users WHERE id = ?').get(user.id);
    ...
    fastify.post('/auth/register', {
        ...
        const { username, isGuest, password, email } = request.body;
        if (!username) {
            return reply.code(400).send({
                success: false,
