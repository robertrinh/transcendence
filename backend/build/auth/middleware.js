...
const extractVerifyToken = (request, reply) => {
...
export const authenticatePendingOnly = async (request, reply) => {
    const payload = extractVerifyToken(request, reply);
    if (!payload)
        return;
    if (!payload.twoFactorPending) {
...
export const requireNonGuest = async (request, reply) => {
    ...
    const row = db.prepare('SELECT is_guest FROM users WHERE id = ?').get(request.user.userId);
    if (!row) {
        return reply.code(401).send({
            success: false,
            error: 'Account no longer exists'
