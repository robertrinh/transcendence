...
export const tournamentController = {
    ...
    createTournament: async (req, reply) => {
    ...
    joinTournament: async (req, reply) => {
        ...
        if (participants.length + 1 === tournament.max_participants) {
            const allParticipants = tournamentService.getTournamentParticipants(id);
    ...
    leaveTournament: async (req, reply) => {
        const { id } = req.params;
        const user_id = req.user.userId;
    ...
    getTournamentParticipants: async (req, reply) => {
        const { id } = req.params;
        const participants = tournamentService.getTournamentParticipants(id);
    ...
    getTournamentGames: async (req, reply) => {
        const { id } = req.params;
        const games = tournamentService.getTournamentGames(id);
        if (games.length === 0)
